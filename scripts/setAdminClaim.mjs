...
if (!serviceAccountStr) {
  ...
  console.error(
    "Set it to the contents of your serviceAccountKey.json, or a path to the file.",
  );
  process.exit(1);
}
...
try {
  ...
  try {
    serviceAccount = JSON.parse(
      readFileSync(path.resolve(serviceAccountStr), "utf8"),
    );
  } catch (e) {
    console.error(
      "ERROR: Could not parse FIREBASE_SERVICE_ACCOUNT as JSON or file path:",
      e.message,
    );
    process.exit(1);
  }
...
try {
  ...
  console.log(
    "   The user must sign out and re-authenticate for the claim to take effect in their ID token.",
  );
} catch (err) {
