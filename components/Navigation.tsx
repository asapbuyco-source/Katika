...
export const Navigation: React.FC<NavigationProps> = ({ currentView, setView, user, hasUnreadMessages }) => {
  ...
  const navItems: { id: string; icon: any; label: string; hasBadge?: boolean }[] = [
      ...
      <div className="flex justify-between md:justify-around items-center md:flex-col md:gap-8 w-full px-2 md:px-0">
        {navItems.map((item) => (
          <button
            ...
            className={`flex flex-col items-center gap-1 transition-all relative group flex-1 md:flex-none ${currentView === item.id ? 'text-gold-400' : 'text-slate-400 hover:text-white'
              }`}
          >
        ...
        <button
          ...
          className={`flex flex-col items-center gap-1 transition-all relative group flex-1 md:flex-none ${currentView === 'profile' ? 'text-gold-400' : 'text-slate-400 hover:text-white'
            }`}
        >
          <div className={`p-2 rounded-xl transition-all ${currentView === 'profile' ? 'bg-royal-800' : 'group-hover:bg-royal-800/50'}`}>
            <User size={22} className={currentView === 'profile' ? 'drop-shadow-[0_0_8px_rgba(251,191,36,0.5)]' : ''} />
          </div>
          <span className="text-[9px] md:text-[10px] font-medium tracking-wide">{t('nav_profile')}</span>
          {currentView === 'profile' && (
            <div className="absolute -right-3 top-1/2 -translate-y-1/2 w-1 h-8 bg-gold-500 rounded-l-full hidden md:block" />
          )}
        </button>
        ...
        {user.isAdmin && (
          <button
            ...
            className={`flex flex-col items-center gap-1 transition-all relative group flex-1 md:flex-none ${currentView === 'admin' ? 'text-red-400' : 'text-slate-500 hover:text-red-300'
              }`}
          >
            <div className={`p-2 rounded-xl transition-all ${currentView === 'admin' ? 'bg-red-500/20' : 'group-hover:bg-red-500/10'}`}>
              <ShieldAlert size={22} className={currentView === 'admin' ? 'drop-shadow-[0_0_8px_rgba(239,68,68,0.5)]' : ''} />
            </div>
            <span className="text-[9px] md:text-[10px] font-bold tracking-wide">{t('nav_admin')}</span>
            {currentView === 'admin' && (
