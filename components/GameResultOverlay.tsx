...
export const GameResultOverlay: React.FC<GameResultOverlayProps> = ({
    ...
    return (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-6 bg-black/80 backdrop-blur-md">
                ...
                <div className="absolute inset-0 overflow-hidden pointer-events-none">
                    {particles.map((_, i) => (
                        <motion.div
                            ...
                            className={`absolute w-3 h-3 rounded-sm ${['bg-gold-400', 'bg-cam-red', 'bg-cam-green', 'bg-purple-400'][Math.floor(Math.random() * 4)]
                                }`}
                        />
                    ))}
                </div>
            ...
            >
                {/* Glow Effect */}
                <div className={`absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[120%] h-[120%] rounded-full blur-[100px] -z-10 ${result === 'win' ? 'bg-gold-500/30' : result === 'draw' ? 'bg-slate-500/20' : 'bg-red-500/20'
                    }`} />

                <div className={`
            glass-panel rounded-3xl p-1 border-2 overflow-hidden shadow-2xl relative
            ${result === 'win' ? 'border-gold-500/50 bg-royal-900/90' : result === 'draw' ? 'border-slate-500/30 bg-royal-900/90' : 'border-red-500/30 bg-black/90'}
        `}>
                    <div className="relative p-8 flex flex-col items-center text-center z-10">
                        ...
                        {/* Text Content */}
                        <h2 className={`text-4xl font-display font-black uppercase mb-2 ${result === 'win' ? 'text-transparent bg-clip-text bg-gradient-to-b from-white to-gold-400' : 'text-slate-200'
                            }`}>
                            {result === 'win' ? (isTournament ? 'Round Cleared!' : 'Victory!') : result === 'draw' ? "It's a Draw!" : result === 'quit' ? 'Quit' : 'Defeat'}
                        </h2>

                        <p className="text-slate-400 mb-8 text-sm">
                            {isTournament ? (
                                result === 'win' ? 'You have advanced to the next bracket.' : 'Eliminated. Thank you for participating.'
                            ) : result === 'draw' ? 'A perfectly balanced match. No stakes were exchanged.' : (
